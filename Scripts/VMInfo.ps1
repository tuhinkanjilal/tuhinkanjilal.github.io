...
$MyReport = Get-ClusterResourceSummary "Monthly Report - VMware Cluster Resource Summary"
	
	if ($ShowClusterResourceSummary){
		Write-ToConsole "... Starting Monthly Report - VMware Cluster Resource Summary"
...
if ($DisplayToScreen) {
	Write-ToConsole "... Displaying Monthly Report - VMware Cluster Resource Summary"
	if (-not (test-path C:\temp\vmreporttesting\)){
		MD C:\temp\vmreporttesting | Out-Null
	}
	$Filename = "C:\temp\Monthly-VMwareClusterResourceSummary" + "_" + $Date.Month + "-" + $Date.Day + "-" + $Date.Year + "_" + $Date.Hour + "-" + $Date.Minute + "-" + $Date.Second + ".htm"
	$MyReport | out-file -encoding ASCII -filepath $Filename
	Invoke-Item $Filename
}

if ($SendEmail) {
	Write-ToConsole "... Sending Monthly Report - VMware Cluster Resource Summary to $EmailTo"
	send-SMTPmail $EmailTo $EmailFrom "Monthly Report - VMware Cluster Resource Summary" $SMTPServer $MyReport
}
